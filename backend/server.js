...
const PUBLIC_SITE = require('./lib/public-site').publicSite(process.env.PUBLIC_SITE_URL);
      ...
      if (p === '/admin' || p.startsWith('/api/admin')) {
        ...
        if (p === '/api/admin/invitations/batches') {
          ...
          try {
            ...
            if (req.method === 'POST') {
              const body = await readBody(req, 3 * 1024 * 1024);
              const saved = body.action==='move-unassigned' ? await batches.moveUnassigned(body.id,body.target,body.ids) : body.action==='import-guests' ? await batches.importGuests(body.id,body.csv,body.commit===true) : body.action==='assign' ? await batches.add(body.id,body.ids,body.type,true) : body.action==='create' ? await batches.create(body.name,actor) : body.action==='add' ? await batches.add(body.id,body.ids,body.type) : await batches.save(body.csv, actor);
