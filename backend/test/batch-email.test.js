...
test('batch email approval, queue uniqueness, delivery states and retry safety',async()=>{
 ...
 try{
  ...
  const attendance=require('../lib/attendance').make(db,vault);
  const token=new URL(draft.text.match(/https:\/\/x-peng.netlify.app\/confirm.html#[^\s]+/)[0]).hash.slice(1);
  a.equal((await attendance.confirm(token)).ok,false);
  a.equal((await attendance.confirm('preview')).ok,false);
  a.equal((await db.get('SELECT COUNT(*) AS n FROM attendance_confirmations')).n,0);
  ...
  await service.test(id,'team@example.com','admin');a.match(calls[0].text,/TEST-NOT-VALID/);a.ok(!calls[0].text.includes('VIP0001'));a.match(calls[0].html,/SM Tickets outlet/);a.match(calls[0].text,/confirm.html#preview/);
  ...
  const sentToken=new URL(calls[1].text.match(/https:\/\/x-peng.netlify.app\/confirm.html#[^\s]+/)[0]).hash.slice(1);
  a.equal((await attendance.confirm(sentToken.slice(0,-1)+(sentToken.endsWith('a')?'b':'a'))).ok,false);
  const confirmed=await attendance.confirm(sentToken);a.equal(confirmed.ok,true);
  a.deepEqual(await attendance.confirm(sentToken),confirmed);
  a.equal((await db.get('SELECT COUNT(*) AS n FROM attendance_confirmations')).n,1);
  a.equal((await service.state(id,'admin')).rows.filter(r=>r.confirmed_at).length,1);
  a.equal((await db.get("SELECT COUNT(*) AS n FROM invitations WHERE status='CLAIMED'")).n,0);
